...
module.exports = function mongooseCacheBox(mongoose, options, cb) {
  ...
  var TTL = options.ttl || 60
    ...
    //, find = Query.prototype.find
    , execFind = Query.prototype.execFind;

  /*Query.prototype.find = function (condition, fn) {
    //find.call(this, condition, fn);
    
    var query = this;
    var obj = {};

    ['op', 'options', '_conditions', '_fields', '_path', '_distinct'].forEach(function (key) {
      obj[key] = query[key];
    });

    obj.model = this.model.modelName;
    obj.collection = this._collection.collection.name;
    this.key = JSON.stringify(obj);

    this.initCache(this.model.schema.options);
